...
async function uploadImageToFirebase(imageUrl, fileName, folderName, retryCount = 3) {
  try {
    const imageResponse = await fetch(imageUrl, {
      headers: {
        'Authorization': authHeader,
    ...
    console.error('Error uploading image to Firebase:', error);
    if (retryCount > 0) {
      console.log(`Retrying upload for ${fileName}. Retries left: ${retryCount - 1}`);
      // Exponential backoff: wait before retrying
      await new Promise(resolve => setTimeout(resolve, Math.pow(2, 4 - retryCount) * 1000));
      return uploadImageToFirebase(imageUrl, fileName, folderName, retryCount - 1);
    } else {
      throw error;
    }
  }
...
async function fetchAttachments(childPageId) {
  try {
    const attachmentsResponse = await fetch(
      `https://${domain}.atlassian.net/wiki/rest/api/content/${childPageId}/child/attachment`,
      // https://go-jek.atlassian.net/wiki/rest/api/content/2846951349/child/attachment
